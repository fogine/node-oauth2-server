    ...
    .spread(function(expiresAt, client, user) {
      ...
      return Promise.bind(this)
        .then(function() {
          ...
          return this.validateScope(user, client, requestedScope, request);
        })
        .then(function(validScope) {
          scope = validScope;

          return this.generateAuthorizationCode(client, user, scope, request);
        })
        .then(function(authorizationCode) {
          state = this.getState(request);
          ResponseType = this.getResponseType(request);

          return this.saveAuthorizationCode(authorizationCode, expiresAt, scope, client, uri, user, request);
        })
        .then(function(code) {
...
AuthorizeHandler.prototype.generateAuthorizationCode = function(client, user, scope, request) {
  if (this.model.generateAuthorizationCode) {
    return promisify(this.model.generateAuthorizationCode, 4).call(this.model, client, user, scope, request);
  }
 ...
 */
AuthorizeHandler.prototype.validateScope = function(user, client, scope, request) {
  if (this.model.validateScope) {
    return promisify(this.model.validateScope, 4).call(this.model, user, client, scope, request)
      .then(function (scope) {
        if (!scope) {
...
AuthorizeHandler.prototype.saveAuthorizationCode = function(authorizationCode, expiresAt, scope, client, redirectUri, user) {
  ...
  return promisify(this.model.saveAuthorizationCode, 4).call(this.model, code, client, user);
};
